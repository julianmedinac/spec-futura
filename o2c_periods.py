"""
DOR Framework - Standard Multi-Period Analysis
Produces distribution tables, statistics, histograms, and sigma tables
matching the NQ DOR PRO format.

Supports daily O2C and weekly O2C (Monday Open to Friday Close).
"""
import sys
sys.path.insert(0, '.')
...
# Bin configuration per timeframe
BIN_CONFIG = {
    'daily': {'size': 0.005, 'min': -0.03, 'max': 0.03},       # 0.50%, -3% to +3%
    'weekly': {'size': 0.005, 'min': -0.06, 'max': 0.06},      # 0.50%, -6% to +6%
    'monthly': {'size': 0.01, 'min': -0.12, 'max': 0.12},      # 1.00%, -12% to +12%
}

TIMEFRAME_LABELS = {
    'daily': 'OPEN TO CLOSE (DIARIO)',
    'weekly': 'OPEN TO CLOSE (SEMANAL)',
    'monthly': 'OPEN TO CLOSE (MENSUAL)',
}


def analyze_period(asset_key, start, end, timeframe='daily'):
    """Download data and compute O2C returns for a period."""
    data = download_asset_data(asset_key, start_date=start, end_date=end)

    if timeframe == 'daily':
        # Daily: (Close - Open) / Open
        o2c = (data['close'] - data['open']) / data['open']
        o2c = o2c.dropna()
        o2c.name = 'o2c_daily'
    elif timeframe == 'weekly':
        # Weekly: (Friday Close - Monday Open) / Monday Open
        weekly = data.resample('W').agg({
            'open': 'first',    # Monday open
            'high': 'max',
            'low': 'min',
        ...
        # Monthly: (Last Close - First Open) / First Open
...
def build_distribution_table(o2c, timeframe='daily'):
    """Build frequency distribution table."""
    cfg = BIN_CONFIG[timeframe]
    bin_size = cfg['size']
    bin_min = cfg['min']
    bin_max = cfg['max']
    edges = np.arange(bin_min, bin_max + bin_size, bin_size)
    # Clean floating point: round to avoid 0.0050000000000001 issues
...
def print_text_report(o2c, label, asset_key, timeframe='daily'):
    """Print report to console."""
    tf_label = TIMEFRAME_LABELS[timeframe]
    dist_table = build_distribution_table(o2c, timeframe)
    stats_dict = build_stats(o2c)
    sigma_table = build_sigma_table(o2c)

    print()
    print("=" * 90)
    print(f"  {asset_key} - {tf_label}  |  Periodo: {label}  |  Cuenta: {len(o2c)}")
    print("=" * 90)
...
def generate_chart(o2c, label, asset_key, output_dir, timeframe='daily'):
    """Generate chart matching the Excel format."""
    tf_label = TIMEFRAME_LABELS[timeframe]
    tf_short = {'daily': 'D', 'weekly': 'W', 'monthly': 'M'}[timeframe]
    dist_table = build_distribution_table(o2c, timeframe)
    stats_dict = build_stats(o2c)
    sigma_table = build_sigma_table(o2c)
    ...
    fig.suptitle(f"{asset_key} - {tf_label}  |  {label}", fontsize=16, fontweight='bold', y=0.98)
    ...
    ax_hist.set_xticks(x_pos[::2])  # Show every other label to avoid crowding
    ax_hist.set_xticklabels(bar_labels[::2], rotation=45, ha='right', fontsize=7)
    ax_hist.set_ylabel('Probabilidad %')
    ax_hist.set_title(tf_label, fontweight='bold', fontsize=12)
    ...
    # Save to output/charts/[ASSET]/[timeframe]/ structure
    output_path = Path(output_dir) / asset_key / timeframe
    output_path.mkdir(parents=True, exist_ok=True)

    chart_name = f"DOR_O2C_{tf_short}_{asset_key}_{label}_{datetime.now().strftime('%Y%m%d')}.png"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='DOR - Standard Multi-Period O2C Analysis')
    ...
    for start, end, label in PERIODS:
        print(f"\n{'='*60}")
        print(f"Processing {asset_key} ({timeframe}) - {label}...")
        print(f"{'='*60}")

        o2c = analyze_period(asset_key, start, end, timeframe)
        print_text_report(o2c, label, asset_key, timeframe)
        generate_chart(o2c, label, asset_key, args.output, timeframe)

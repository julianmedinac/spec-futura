...
def analyze_gspc_q1_high_tdoq_18():
    loader = DataLoader()
    print("Downloading GSPC data (Max History)...")
    # Get Max History for S&P 500
    data = loader.download('GSPC', start_date='1928-01-01')
    ...
    print(f"\nScanning {len(unique_years)} years for GSPC Q1 High at TDOQ ~18 (Range 16-20)...")
    ...
    for year in unique_years:
        ...
        # This corresponds to roughly Jan 24-30
        if 16 <= high_tdoq <= 20:
            
            # Additional Context: Was Jan High the Yearly High?
            full_year_data = data[data['year'] == year]
            yearly_high = full_year_data['high'].max()
            is_yearly_high = (q1_high_val == yearly_high)
            
            # Did we break that Q1 High later in the year?
    ...
    print("\n" + "="*60)
    print(f"RESULTS: GSPC Q1 High occurring at TDOQ 16-20 (Late Jan)")
    print(f"Sample Size: {len(df_res)} years (out of {len(unique_years)})")
    print("="*60)
    ...
    print("\nMatching Years:")
    print(df_res[['year', 'high_tdoq', 'high_date', 'is_yearly_high', 'broke_high_later', 'year_return', 'drawdown_from_high']].to_string(index=False))
    
    # Statistics
    print("\n--- STATISTICS ---")
    prob_break = df_res['broke_high_later'].mean() * 100
    prob_yearly_high_is_in = df_res['is_yearly_high'].mean() * 100
    avg_return = df_res['year_return'].mean() * 100
    avg_dd = df_res['drawdown_from_high'].mean() * 100
    
    print(f"Prob(Eventually Breaking Q1 High): {prob_break:.1f}%")
    print(f"Prob(That Q1 High remains YEARLY HIGH): {prob_yearly_high_is_in:.1f}%")
    print(f"Average Yearly Return: {avg_return:.2f}%")
    print(f"Average Drawdown from High: {avg_dd:.2f}%")
    
    # Bearish Cases
    bearish_years = df_res[df_res['year_return'] < 0]
    print(f"\nBearish Years count: {len(bearish_years)} ({len(bearish_years)/len(df_res):.0%})")
    
    # Highlight 2026 GSPC status
    print(f"\nNote: In 2026, GSPC Q1 High was Jan 27 (TDOQ 18 approx).")

if __name__ == "__main__":
    analyze_gspc_q1_high_tdoq_18()
